...
const getTravelsPerDay = (req, res) => {
    ...
    mysqlConnection.query(
        ` SELECT COUNT(*) AS COUNT, DAYNAME(DATE(vd.create_at)) AS DATE`+ 
        ` FROM viajes_drivers vd`+
        ` JOIN users_drivers ud`+
        ` ON vd.placa_conductor = ud.placa_car`+
        ` WHERE vd.create_at >= date_sub(CURDATE(), INTERVAL 60 day) ${createdFilterConditional()}${provinceFilterConditional()}${departmentFilterConditional()}${districtFilterConditional()}`+
        ` GROUP BY date `
        , (err, rows, fields) => {
            if (!err) {
                res.json(rows)
...
export default getTravelsPerDay;
